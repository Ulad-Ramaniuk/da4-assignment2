...
tp_log = -b1 / (2 * b2)
print(f"EKC Turning point: log GDP = {tp_log:.3f} → GDP per capita = ${np.exp(tp_log):,.0f} PPP")
...
# =============================================================================
# 9. Model 6 EKC + Energy Confounder
# =============================================================================

model6_nl_energy = PanelOLS.from_formula(
    'log_co2 ~ log_gdp + log_gdp_sq + log_energy + EntityEffects + TimeEffects',
    data=df_fe
).fit(cov_type='clustered', cluster_entity=True)

print(f"\n--- Model 6 EKC + Energy (N={model6_nl_energy.nobs}) ---")
print(f"log_gdp:     {model6_nl_energy.params['log_gdp']:.4f}  SE={model6_nl_energy.std_errors['log_gdp']:.4f}  p={model6_nl_energy.pvalues['log_gdp']:.4f}")
print(f"log_gdp_sq:  {model6_nl_energy.params['log_gdp_sq']:.4f}  SE={model6_nl_energy.std_errors['log_gdp_sq']:.4f}  p={model6_nl_energy.pvalues['log_gdp_sq']:.4f}")
print(f"log_energy:  {model6_nl_energy.params['log_energy']:.4f}  SE={model6_nl_energy.std_errors['log_energy']:.4f}  p={model6_nl_energy.pvalues['log_energy']:.4f}")
print(f"Within R²: {model6_nl_energy.rsquared_within:.3f}")

b1 = model6_nl_energy.params['log_gdp']
b2 = model6_nl_energy.params['log_gdp_sq']
tp = np.exp(-b1 / (2 * b2))
print(f"EKC Turning point with energy: ${tp:,.0f} PPP")

# =============================================================================
# 10. APPENDIX: ADDITIONAL CONFOUNDERS & EXTENSIONS
# =============================================================================
...
# =============================================================================
# 12. UNIT ROOT & COINTEGRATION TESTS
# =============================================================================
print("\n--- Unit Root Tests (ADF) ---")
